	...
	void Update () {
        ...
        if (pf.path != null)
        {
            if (pf.path.Count > 0) {
                ...
                transform.LookAt(new Vector3(target.worldPosition.x, 1.0f, target.worldPosition.z));
                ...
                transform.position += (target.worldPosition + new Vector3(0, 1, 0) - transform.position).normalized * speed * Time.deltaTime;
            }
        }
    }
